...
*** These problems raise the following questions:
 ...
 2) If a post is the only project file, and I go all-out using code blocks (no separate code files), how would version control work?
    1) Would a bags post get an update every time a source code block changes?
	  ...
	  1) Aha! but is there some git mechanism that would help?
	     1) tags
	     2) submodules
 3) Can I work efficiently with code-blocks? At the moment it adds an extra step to the coding process (more emacs-fu required)
 4) Should I make room somewhere for scripts I don't think are worth publishing at all?
 5) Do I want bags to be the process (slighlty polished) or one of many outputs of the process?
